...
class CoreSelectorsTests(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="test.user.1", password=get_random_string(12)
        )
        self.user2 = User.objects.create_user(
            username="test.user.2", password=get_random_string(12)
        )
    ...
    def test_get_user_referrals(self):
        ...
        Referral.objects.create(
            inviter=self.user, invitee=self.user1, code=self.referral_code
        )
        Referral.objects.create(
            inviter=self.user, invitee=self.user2, code=self.referral_code
        )
